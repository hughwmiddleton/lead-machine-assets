...
def test_find_best_page_url_prefers_verified_search_surface_over_feed_when_extractor_handoff_is_locked(monkeypatch) -> None:
...
def test_find_best_page_url_keeps_search_surface_logging_when_verified_surface_is_enforced(monkeypatch) -> None:
    logs = []
    feed_url = "https://www.facebook.com/wrongfeedartist"
    result_url = "https://www.facebook.com/testartistofficial"
    driver = _FakeDriver(
    ...
    def _fake_fetch(query, *, search_method):  # noqa: ANN001
        return (
            (
                ...
                "</div>"
                "<div role='feed'>"
                f"<div class='card'><a href='{feed_url}'>Wrong Feed Artist</a><div class='subtitle'>Musician/Band</div></div>"
                "</div>"
                "</div>"
            ),
            "https://www.facebook.com/search/top/?q=test+artist",
...
def test_find_best_page_url_passes_verified_search_surface_html_to_extractor(monkeypatch) -> None:
    ...
    assert extractor_inputs[0] != page_html
    assert 'data-fb-daytime-lock="1"' in extractor_inputs[0]
    assert artist_url in extractor_inputs[0]
    assert "wrongfeedartist" not in extractor_inputs[0]
...
def test_find_best_page_url_skips_zero_signal_candidate_before_fetch(monkeypatch) -> None:
    artist_url = "https://www.facebook.com/fergie"
    driver = _FakeDriver(
        {
            artist_url: "<html><body><div>Community</div></body></html>",
        }
    )
    client = cde.FacebookSearchClient(driver=driver, logger=None)
    monkeypatch.setattr(client, "ensure_facebook_logged_in", lambda: True)
    monkeypatch.setattr(cde, "score_fb_candidate", lambda *args, **kwargs: (1.0, 0.0, 0.0))
    monkeypatch.setattr(cde, "is_music_page", lambda *args, **kwargs: False)
    monkeypatch.setattr(
        cde,
        "_facebook_candidate_has_min_identity_evidence",
        lambda *args, **kwargs: (False, "identity_floor"),
    )
    ...
    def _fake_fetch(query, *, search_method):  # noqa: ANN001
        return (
            (
                "<div role='main'><div aria-label='Search results'>"
                f"<div class='card'><a href='{artist_url}'>Fergie</a><div class='subtitle'>Community</div></div>"
                "</div></div>"
            ),
            "https://www.facebook.com/search/pages/?q=tallulah+argue",
            False,
...
def test_find_best_page_url_allows_borderline_music_signal_candidate_before_fetch(monkeypatch) -> None:
    artist_url = "https://www.facebook.com/fergie"
    driver = _FakeDriver(
        {
            artist_url: (
