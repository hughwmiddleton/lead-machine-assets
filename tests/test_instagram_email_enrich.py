...
def test_instagram_same_url_success_gate_preserves_no_attempt():
    logs = []
    worker = _make_worker(logs)
    seed_df = _seed_df(
        {
    ...
    _set_email_with_provenance(
        ...
        source_url="https://www.instagram.com/gatedig/",
    ...
    assert seed_df.at[0, cde.IG_WRITE_STATE_COL] == "ig_no_email_written"
    assert seed_df.at[0, cde.IG_TERMINAL_REASON_COL] == "ig_opportunity_not_attempted_same_source_url_success"


def test_instagram_fb_email_does_not_block_ig_scrape(monkeypatch):
    logs = []
    worker = _make_worker(logs)
    seed_df = _seed_df(
        {
            "Artist Name": "FB Existing IG Allowed",
            "Email": "fb@example.com",
            "Email_All": "fb@example.com",
            "Instagram_URL": "https://instagram.com/igallowed/",
        }
    )
    _set_email_with_provenance(
        (seed_df, 0),
        "fb@example.com",
        source_url="https://www.facebook.com/igallowed",
    ...
    def fake_fetch_scope(session, ig_url, retain_live_page=False):  # noqa: ANN001
        yield SimpleNamespace(html="<html><body>ig@example.com</body></html>", status=200)
    ...
    assert logs[0] == "[IG Email] Visiting https://www.instagram.com/igallowed/"
...
class _DummyInstagramHiddenContactPage(_DummyClosable):
    ...
    def evaluate(self, script):  # noqa: ANN001
        ...
        if "ig-live-bio-link-control-surface" in script_text:
            ...
            def add_drop_reason(reason, *, kind="hard"):
                key = str(reason or "").strip()
                if not key:
                    key = "other"
                normalized = key if ":" in key else f"{kind}:{key}"
                drop_reasons[normalized] = drop_reasons.get(normalized, 0) + 1
            ...
            def score_seed(node, scope_root):
                ...
                score = 0
                has_label_url = bool(url_pattern.search(label_text) or bare_domain_pattern.search(label_text))
                has_attr_url = bool(url_pattern.search(attr_text) or bare_domain_pattern.search(attr_text))
                has_positive_label = bool(positive_pattern.search(label_text))
                has_positive_attr = bool(positive_pattern.search(attr_text))
                has_positive_context = bool(positive_pattern.search(context_text))
                has_weak_context = bool(
                    weak_context_pattern.search(context_text) or weak_context_pattern.search(attr_text)
                )
                if has_label_url:
                    score += 10
                if has_attr_url:
                ...
                if score < minimum_score:
                    soft_drop_reasons.append("weak_signal")
                has_strong_signal = (
                    has_label_url
                    or has_attr_url
                    or has_positive_label
                    or has_positive_attr
                    or has_positive_context
                )
                if relaxed_clickable and not has_strong_signal and not weak_context_pattern.search(combined_text):
                    soft_drop_reasons.append("ambiguous_wrapper")
                ...
                candidate_class = "weak_candidate"
                if relaxed_clickable:
                    candidate_class = "interaction_candidate"
                if not soft_drop_reasons and score >= minimum_score and has_strong_signal:
                    candidate_class = "strong_candidate"
                elif not soft_drop_reasons and (
                    not relaxed_clickable or has_strong_signal or score >= minimum_score
                ):
                    candidate_class = "weak_candidate"
                return {
                    "keep": True,
            ...
            for root in scope_roots:
                ...
                for node in root.find_all(["a", "button", "div", "span", "section", "article", "li", "p"]):
                    ...
                    if len(nodes) >= 80:
                        break
                for node in nodes:
                    scored = score_seed(node, root)
                    if not scored:
                        continue
                    raw_candidate_count += 1
                    if not scored.get("keep"):
                        add_drop_reason(scored.get("drop_reason", ""), kind=scored.get("drop_kind", "hard"))
                        continue
                    node_id = id(node)
                    if node_id in seen_candidate_nodes:
                        add_drop_reason("duplicate")
                        continue
                    seen_candidate_nodes.add(node_id)
                    for soft_reason in scored.get("soft_drop_reasons") or []:
                        add_drop_reason(soft_reason, kind="soft")
                    candidate_class = scored.get("candidate_class") or "weak_candidate"
                    if candidate_class == "strong_candidate":
                        strong_candidate_count += 1
                    elif candidate_class == "interaction_candidate":
                        interaction_candidate_count += 1
                    else:
                        weak_candidate_count += 1
                    value_count_before = len(values)
                    add_attr_values(node)
                    add_text_values(node)
                    current = node
                    ...
                    if len(values) == value_count_before and scored.get("interaction_eligible"):
                        add_value(
                            f"{cde._INSTAGRAM_LIVE_ONEHOP_INTERACTION_ONLY_SENTINEL_PREFIX}"
                    ...
                    kept_candidates.append(
                        (
                            ...
                            len(scored.get("soft_drop_reasons") or []),
                            -int(scored.get("score") or 0),
                            str(scored.get("label") or ""),
                        )
                    )
            kept_labels = [entry[3] for entry in sorted(kept_candidates)[:3] if entry[3]]
            return {
                "values": values,
                "rawCandidateCount": raw_candidate_count,
                "keptCandidateCount": strong_candidate_count + weak_candidate_count,
                "interactionCandidateCount": interaction_candidate_count,
...
def _instagram_profile_surface_state_from_html(
    script,
    html,
    *,
    rendered_main_text="",
    ...
    script_text = str(script or "")
    assert "profile_markers" in script_text
    ...
    if not main_text_source and main is not None:
        main_text_source = main.get_text(" ", strip=True)
    main_text = " ".join(str(main_text_source or "").split())
    return {
        "main": 1 if main is not None else 0,
        ...
        "ready": (
            1
            if main is not None and header is not None and profile_markers > 0 and descendants > 0 and len(main_text) >= 16
            else 0
        ),
    }
...
def test_instagram_bridge_surface_assessment_promotes_routed_same_profile_under_rendered_shell():
    html = """
    <html>
      <body>
        <main>
    ...
    page = _DummyInstagramProfileSurfaceProbePage(
        html,
        url="https://www.instagram.com/shellartist/reels/",
        title="shellartist (@shellartist) • Instagram photos and videos",
    )
...
def test_instagram_bridge_surface_assessment_rejects_wrong_profile_under_rendered_shell():
    ...
    page = _DummyInstagramProfileSurfaceProbePage(
        html,
        url="https://www.instagram.com/otherartist/reels/",
...
def test_instagram_bridge_surface_assessment_rejects_generic_plausible_same_handle_surface():
    html = """
    <html>
      <body>
        <main>
          <section>Explore trending creators and recent posts</section>
        </main>
      </body>
    </html>
    """

    class DummyCandidatePage(_DummyInstagramProfileSurfaceProbePage):
        def evaluate(self, script):  # noqa: ANN001
            script_text = str(script or "")
            if "profile_markers" in script_text:
                return _instagram_profile_surface_state_from_html(script, html)
            if "document.body" in script_text and "innerText" in script_text:
                return "Explore trending creators and recent posts"
            return _instagram_profile_surface_candidate_marker_from_html(script, html)

    page = DummyCandidatePage(
        html,
        url="https://www.instagram.com/shellartist/reels/",
        title="shellartist (@shellartist) • Instagram photos and videos",
    )
    ...
    assert assessment["blocked"] is False
    assert assessment["ready"] is False
    assert assessment["plausible_surface"] is True
    assert assessment["promoted_shell"] is False
...
def test_open_instagram_live_page_bridge_non_profile_shell_fails_after_bounded_recovery(
    ...
    class DummyPage(_DummyClosable):
        ...
        def evaluate(self, script):  # noqa: ANN001
            ...
            self.evaluate_calls.append(script_text)
            if "profile_markers" in script_text:
                return _instagram_profile_surface_state_from_html(script, "<html><body><div>Shell only</div></body></html>")
            return False
...
def test_open_instagram_live_page_bridge_logged_out_html_handoff_is_rejected(monkeypatch):
    ...
    class DummyPage(_DummyClosable):
        ...
        def evaluate(self, script):  # noqa: ANN001
            ...
            self.evaluate_calls.append(script_text)
            if "profile_markers" in script_text:
                return _instagram_profile_surface_state_from_html(script, logged_out_html)
            if "document.body" in script_text and "innerText" in script_text:
...
def test_open_instagram_live_page_bridge_non_profile_shell_recovers_after_same_profile_reload(monkeypatch):
    events = []
    shell_html = "<html><body><div>Shell only</div></body></html>"
    ready_html = (
        "<html><body><main><header><h1>Recovered Artist</h1><button>Email</button></header>"
        "<section><a href='https://linktr.ee/recoveredartist'>Bio</a></section></main></body></html>"
    )
    ...
    class DummyPage(_DummyClosable):
        def __init__(self):
            super().__init__()
            self.url = ""
            self._html = shell_html
            self.goto_calls = 0

        def goto(self, url, wait_until=None, timeout=None):  # noqa: ANN001
            self.goto_calls += 1
            self.url = "https://www.instagram.com/recoveredartist/"
            self._html = shell_html if self.goto_calls == 1 else ready_html
            events.append(("goto", self.goto_calls, self.url, wait_until, timeout))

        def evaluate(self, script):  # noqa: ANN001
            script_text = str(script or "")
            if "profile_markers" in script_text:
                rendered_main_text = "" if self._html == shell_html else "Recovered Artist Bio and booking details"
                return _instagram_profile_surface_state_from_html(
                    script,
                    self._html,
                    rendered_main_text=rendered_main_text,
                )
            if "document.body" in script_text and "innerText" in script_text:
                return "" if self._html == shell_html else "Recovered Artist Bio and booking details"
            return _instagram_profile_surface_candidate_marker_from_html(script, self._html)
    ...
    bridge = _REAL_OPEN_INSTAGRAM_LIVE_PAGE_BRIDGE(
        "https://www.instagram.com/recoveredartist/",
        timeout_s=12.5,
    )
...
def test_open_instagram_live_page_bridge_promotes_routed_same_profile_under_rendered_shell(
    monkeypatch,
    capsys,
):
    events = []
    ...
    class DummyPage(_DummyClosable):
        ...
        def goto(self, url, wait_until=None, timeout=None):  # noqa: ANN001
            self.goto_calls += 1
            self.url = "https://www.instagram.com/shellartist/reels/"
            events.append(("goto", self.url, wait_until, timeout))
    ...
    captured = capsys.readouterr()
    assert "[IG Bridge] action=promote_profile_shell" in captured.out
    assert "[IG Bridge] success attempt=1 final_url=https://www.instagram.com/shellartist/reels/" in captured.out
...
def test_collect_instagram_live_profile_clickable_control_values_retains_interaction_only_profile_actions(
    capsys,
):
    page = _DummyInstagramHiddenContactPage(
    ...
    assert values
    assert all(
        value.startswith(cde._INSTAGRAM_LIVE_ONEHOP_INTERACTION_ONLY_SENTINEL_PREFIX)
        for value in values
    )
    captured = capsys.readouterr()
    assert "[IG Detect] candidates raw=" in captured.out
    assert "interaction=" in captured.out
    assert "soft:profile_action:" in captured.out
    assert "interaction_only=" in captured.out


def test_collect_instagram_live_profile_clickable_control_values_preserves_link_in_bio_interaction_candidate(
    capsys,
):
    page = _DummyInstagramHiddenContactPage(
...
def test_collect_instagram_live_profile_clickable_bio_link_urls_runs_interaction_fallback_for_interaction_only_raw_values():
    page = _DummyInstagramHiddenContactPage(
        "<html><body><main><header><div role='button'>link in bio</div><button>Email</button></header></main></body></html>",
        interaction_bio_link_selector=f'[{cde._INSTAGRAM_LIVE_ONEHOP_INTERACTION_MARKER_ATTR}="1"]',
        interaction_bio_link_resolved_url=(
            "https://l.instagram.com/?u=https%3A%2F%2Flinktr.ee%2Finteractiononlyartist&fbclid=abc123"
        ),
    )

    urls = cde._collect_instagram_live_profile_clickable_bio_link_urls(
        page,
        profile_url="https://www.instagram.com/interactiononlyartist/",
        raw_control_values=[
            f"{cde._INSTAGRAM_LIVE_ONEHOP_INTERACTION_ONLY_SENTINEL_PREFIX}nested_clickable:0"
        ],
    )

    assert urls == ["https://linktr.ee/interactiononlyartist"]
    assert page.click_calls == [f'[{cde._INSTAGRAM_LIVE_ONEHOP_INTERACTION_MARKER_ATTR}="1"]']
    assert page.wait_calls == [250]
...
def test_instagram_email_one_hop_live_surface_interaction_recovery_flows_through_existing_ranking(
    ...
    assert any(
        line.startswith(
            ...
            "__ig_interaction_candidate__:nested_clickable"
        )
        for line in logs
    )
    _assert_log_contains(
