...
def test_instagram_email_positive_writes_email_and_provenance(monkeypatch):
    logs = []
...
def test_instagram_email_single_fetch_limit(monkeypatch):
    logs = []
    worker = _make_worker(logs)
    seed_df = _seed_df(
        {
    ...
    def fake_fetch(session, url):
        fetch_calls.append(url)
        return (
            "<html><body><a href='https://example.com/contact'>Contact</a>"
