...
def test_extract_rendered_visible_text_from_driver_exits_early_for_stable_non_email_text(monkeypatch) -> None:
    ...
    monkeypatch.setattr(night_mode_fb.time, "sleep", lambda _seconds: None)

    rendered_text = night_mode_fb._extract_rendered_visible_text_from_driver(driver)

    assert rendered_text == "Artist bio"
    assert driver.execute_calls == 2
    assert driver.wait_configs == []
...
def test_extract_rendered_visible_text_from_driver_returns_after_follow_up_for_non_material_growth(monkeypatch) -> None:
...
def test_extract_rendered_visible_text_from_driver_keeps_material_growth_exit(monkeypatch) -> None:
    ...
    assert driver.wait_configs == [(pytest.approx(1.3), 0.2)]


def test_extract_rendered_visible_text_from_driver_always_takes_two_snapshots_before_return(monkeypatch) -> None:
    driver = _SequencedRenderedTextDriver(["Immediately usable text", "Immediately usable text"])
    monkeypatch.setattr(night_mode_fb, "WebDriverWait", _StubWebDriverWait)
    monkeypatch.setattr(night_mode_fb.time, "sleep", lambda _seconds: None)

    rendered_text = night_mode_fb._extract_rendered_visible_text_from_driver(driver)

    assert rendered_text == "Immediately usable text"
    assert driver.execute_calls == 2
    assert driver.wait_configs == []


def test_extract_rendered_visible_text_from_driver_avoids_long_wait_for_continuous_non_meaningful_jitter(monkeypatch) -> None:
    driver = _SequencedRenderedTextDriver(
        [
            "Artist bio and links",
            "Artist bio and links updated",
            "Artist bio and links updated again",
            "Artist bio and links updated again and again",
        ]
    )
    monkeypatch.setattr(night_mode_fb, "WebDriverWait", _StubWebDriverWait)
    monkeypatch.setattr(night_mode_fb.time, "sleep", lambda _seconds: None)
    ...
    assert rendered_text == "Artist bio and links updated"
    assert driver.execute_calls == 2
    assert driver.wait_configs == []
...
def test_classify_explicit_fb_intake_attempt_with_promotion_gap() -> None:
    ...
    assert decision.outcome == "promotion_expected_missing_canonical"
...
def test_pass_a_uses_canonical_url_even_when_source_fields_still_contain_share(monkeypatch) -> None:
    logs = []
    enricher = night_mode_fb.NightModeFacebookEnricher(
        legacy_module=None,
        username="",
    ...
    monkeypatch.setattr(
        enricher,
        "_search_for_page",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("canonical explicit URL should not enter discovery")),
    )
    ...
    row = {
        ...
        "Social Link": "https://www.facebook.com/share/19bactwuev?mibextid=wwXIfr",
        "External Links": "https://www.facebook.com/share/another?mibextid=wwXIfr",
        "Email": "",
        "Email_All": "",
...
@pytest.mark.parametrize(
    "source_field",
    ["Social Link", "External Links"],
)
def test_prepare_explicit_fb_row_for_intake_does_not_fallback_to_source_fields(monkeypatch, source_field: str) -> None:
    enricher = night_mode_fb.NightModeFacebookEnricher(
        legacy_module=None,
        username="",
        password="",
    ...
    monkeypatch.setattr(
        enricher,
        "_ensure_session",
        lambda: (_ for _ in ()).throw(AssertionError("source fallback must not resolve via session")),
    )

    row = {
        "Artist Name": "Bad Share Target",
        source_field: "https://www.facebook.com/share/19bactwuev?mibextid=wwXIfr",
    }

    prepared_row, authed_session_available = enricher._prepare_explicit_fb_row_for_intake(row)
    prepared_urls = night_mode_fb.explicit_fb_entrypoint_urls_for_row(prepared_row)
    decision = night_mode_fb.classify_explicit_fb_intake(prepared_row, accepted_urls=prepared_urls)

    assert authed_session_available is None
    assert prepared_row.get("Facebook_URL", "") == ""
...
def test_load_fb_page_with_timeout_unblocks_on_valid_url_change_with_min_ready_probe(monkeypatch) -> None:
    class _Clock:
        def __init__(self) -> None:
            self.now = 100.0
    ...
    class _Driver:
        ...
        def get(self, url):  # noqa: ANN001
            self.get_calls += 1
            raise AssertionError("driver.get should not be used when min-ready handoff succeeds")
        ...
        def execute_script(self, script, *args):  # noqa: ANN001
            ...
            if "fb_nav_surface_probe" in script:
                self.surface_probe_checks += 1
                raise RuntimeError("surface probe unavailable during JS handoff")
            if script == "window.stop();":
                self.stop_called = True
                return None
            raise AssertionError(f"unexpected script: {script}")
    ...
    assert html == "<html><body>Artist page</body></html>"
    assert driver.get_calls == 0
    assert driver.surface_probe_checks >= 1
    assert driver.stop_called is False
...
def test_load_fb_page_with_timeout_unblocks_on_min_ready_with_usable_handoff_url(monkeypatch) -> None:
    class _Clock:
        def __init__(self) -> None:
            self.now = 100.0
    ...
    class _Driver:
        def __init__(self) -> None:
            self.current_url = "https://www.facebook.com/example"
            self.page_source = "<html><body>Original</body></html>"
            self.nav_target = ""
            self.min_ready_checks = 0
            self.stop_called = False
        ...
        def get(self, url):  # noqa: ANN001
            raise AssertionError("driver.get should not be used when minimal-ready handoff succeeds")
    ...
    assert timed_out is False
    assert current_url == "https://www.facebook.com/example"
    assert html == "<html><body>Original</body></html>"
    assert driver.stop_called is False
    assert clock.now < 101.0
...
def test_load_fb_page_with_timeout_waits_on_root_handoff_url(monkeypatch) -> None:
    class _Clock:
        def __init__(self) -> None:
            self.now = 100.0
        ...
        def execute_script(self, script, *args):  # noqa: ANN001
            ...
            if "fb_nav_min_ready_probe" in script:
                ...
                if self.min_ready_checks >= 2:
                    ...
                    self.page_source = "<html><body>Homepage</body></html>"
                    return ["interactive", True]
                return ["loading", False]
            if "fb_nav_surface_probe" in script:
                return ["loading", "", 0, 0]
    ...
    assert html == "<html><body>Homepage</body></html>"
    assert driver.stop_called is True
...
def test_load_fb_page_with_timeout_unblocks_on_ready_html_before_url_changes(monkeypatch) -> None:
    class _Clock:
        def __init__(self) -> None:
            self.now = 100.0
    ...
    class _Driver:
        ...
        def execute_script(self, script, *args):  # noqa: ANN001
            ...
            if "fb_nav_min_ready_probe" in script:
                self.min_ready_checks += 1
                if self.min_ready_checks >= 2:
                    self.page_source = "<html><body>Artist page with contact details</body></html>"
                    return ["interactive", True]
                return ["loading", False]
            if "fb_nav_surface_probe" in script:
                return ["loading", "", 0, 0]
    ...
    assert timed_out is False
    assert current_url == "https://www.facebook.com/original"
    assert html == "<html><body>Artist page with contact details</body></html>"
    assert driver.stop_called is False
    assert clock.now < 101.0


def test_load_fb_page_with_timeout_waits_when_ready_html_is_still_trivial(monkeypatch) -> None:
    class _Clock:
        def __init__(self) -> None:
            self.now = 100.0
...
def test_load_fb_page_with_timeout_polling_loop_does_not_repeatedly_read_page_source(monkeypatch) -> None:
    ...
    assert current_url == "https://www.facebook.com/example"
    assert html == "<html><body>Artist page</body></html>"
    assert driver.page_source_reads == 3


def test_load_fb_page_with_timeout_preserves_timeout_salvage_when_no_valid_handoff(monkeypatch) -> None:
    class _Clock:
        def __init__(self) -> None:
            self.now = 100.0
