...
class RecoverySummary:
    ...
    def as_dict(self) -> Dict[str, int]:
        return {
            ...
            "fb_attempted": self.fb_attempted,
...
def row_qualifies_for_recovery(row: Mapping[str, Any]) -> bool:
    ...
    if not raw_share:
        return False
    if _cell(row.get(FB_OPPORTUNITY_STATE_COL, "")).lower() != "no_fb_opportunity":
        return False
    if _cell(row.get(FB_ATTEMPT_STATE_COL, "")).lower() != "fb_not_attempted":
        return False
...
def _append_debug_reason(existing: str, reason: str) -> str:
    existing = _cell(existing)
    if not existing:
        return reason
    parts = [part.strip() for part in re.split(r"[|;]", existing) if part.strip()]
    if reason in parts:
        return existing
...
def recover_dataframe(
    ...
    *,
    ...
    if "FB_Status" not in out.columns:
        out["FB_Status"] = ""
    for col in FB_SHARE_TRACE_COLUMNS:
    ...
    try:
        for idx in out.index:
            ...
            raw_fb_url, _source = find_fb_share_url(row)
            out.at[idx, "raw_fb_url"] = raw_fb_url
            out.at[idx, "resolution_attempted"] = "true"

            resolved_raw = resolver(raw_fb_url)
            resolved_fb_url = canonicalize_facebook_url(resolved_raw)
            if not resolved_fb_url:
                out.at[idx, "resolved_fb_url"] = ""
                out.at[idx, "resolution_success"] = "false"
                out.at[idx, FB_GATE_STATE_COL] = "fb_share_resolution_failed"
                out.at[idx, FB_DEBUG_REASON_COL] = _append_debug_reason(
                    out.at[idx, FB_DEBUG_REASON_COL],
                    "fb_share_resolution_failed",
                )
                summary.resolution_failed += 1
                if not out.loc[idx].equals(before):
                    changed_rows.add(int(idx))
                continue

            summary.resolution_success += 1
            out.at[idx, "resolved_fb_url"] = resolved_fb_url
            out.at[idx, "resolution_success"] = "true"
            ...
            for key, value in working.items():
                if key not in out.columns:
                    out[key] = ""
                if key in FB_SHARE_TRACE_COLUMNS or key in FB_RESULT_COPY_COLUMNS or key in DIRECT_FB_ALIAS_FIELDS:
                    out.at[idx, key] = value
            out.at[idx, FB_OPPORTUNITY_STATE_COL] = "fb_opportunity_present"
            out.at[idx, FB_ATTEMPT_STATE_COL] = "attempted_fb"

            summary.fb_attempted += 1
            ...
            if enriched:
                ...
                for key, value in merged.items():
                    if key not in out.columns:
                        out[key] = ""
                    if key in FB_SHARE_TRACE_COLUMNS or key in FB_RESULT_COPY_COLUMNS or key in EMAIL_WRITE_COLUMNS or str(key).startswith("FB_"):
                        out.at[idx, key] = value
            out.at[idx, FB_OPPORTUNITY_STATE_COL] = "fb_opportunity_present"
            out.at[idx, "raw_fb_url"] = raw_fb_url
            out.at[idx, "resolved_fb_url"] = resolved_fb_url
...
def recover_csv(
    ...
    *,
    ...
    recovered, summary = recover_dataframe(df, share_resolver=share_resolver, fb_enricher=fb_enricher)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    summary = recover_csv(args.input, output, in_place=bool(args.in_place))
